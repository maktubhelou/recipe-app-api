...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_filter_recipe_by_tags(self):
        ...
        recipe1 = sample_recipe(
            user=self.user, title='Vietnamese Chicken Curry')
        recipe2 = sample_recipe(user=self.user, title='Pad Thai')
        tag1 = sample_tag(user=self.user, name='Curry')
        tag2 = sample_tag(user=self.user, name='Street Food')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = sample_recipe(user=self.user, title="Chinese Fried Rice")
